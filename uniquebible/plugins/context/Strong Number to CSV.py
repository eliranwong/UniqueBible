...
if config.pluginContext:
    if re.match("^[GH][0-9]+?$", config.pluginContext):
        generate = subprocess.Popen("{0} {1} {2}".format(sys.executable, os.path.join(config.packageDir, "plugins", "context", "Strongs2csv", "strong2csv.py"), config.pluginContext), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        *_, stderr = generate.communicate()
        outputFile = os.path.join(config.packageDir, "plugins", "context", "Strongs2csv", "{0}.csv".format(config.pluginContext))
