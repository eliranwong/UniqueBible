...
class BiblesSqlite:
    ...
    # to-do-list
    # a script to help current users combine bibles of two formats - half-done, yet to put in start-up script
    # put an empty KJV table in bibles.sqlite
    # keep Details, OHGB, OHGBi, LXX?, title in bibles.sqlite
    # port functions from BiblesSqlite to BibleSqlite; completed: readTranslations; yet to complete ......; next: compareVerseChapter
    # redirect related text commands
    # change in ThirdParty
    ...
    def getFormattedBibleList(self):
        ...
        return formattedBibles

    def combinePlainFormattedBibles(self):
        plainBilbes = self.getBibleList()
        formattedBibles = self.getFormattedBibleList()
        biblesWithBothVersions = list(set(plainBilbes) & set(formattedBibles))
    ...
    def getMenu(self, command, source="main"):
        ...
        if not text == "":
            ...
            if bcList:
                ...
                if check == 3:
                    if source == "main":
                        anotherView = "<button class='feature' onclick='document.title=\"STUDY:::{0}:::{1}\"'>go to right view</button>".format(text, mainVerseReference)
                    elif source == "study":
                        anotherView = "<button class='feature' onclick='document.title=\"MAIN:::{0}:::{1}\"'>go to left view</button>".format(text, mainVerseReference)
    ...
    def readTranslations(self, b, c, v, texts):
        biblesSqliteTexts = ["OHGB", "OHGBi", "LXX"]
        individualBibleList = self.getFormattedBibleList()
        exclude = ("LXX", "LXX1", "LXX1i", "LXX2", "LXX2i", "MOB", "MAB", "MIB", "MPB", "MTB", "OHGB", "OHGBi")
        individualTexts = [bible for bible in individualBibleList if not bible in exclude]

        if texts == ["ALL"]:
            texts = biblesSqliteTexts + individualTexts

        verses = "<h2>{0}</h2>".format(self.bcvToVerseReference(b, c, v))
        for text in texts:
            if text in biblesSqliteTexts:
                book, chapter, verse, verseText = self.readTextVerse(text, b, c, v)
            else:
                book, chapter, verse, verseText = Bible(text).readTextVerse(b, c, v)
            divTag = "<div>"
            if b < 40 and text in config.rtlTexts:
...
class Bible:
    ...
    def importPlainFormat(self, verses):
        query = "SELECT name FROM sqlite_master WHERE type='table' AND name=?"
